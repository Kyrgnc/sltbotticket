...
export const data = {
    name: "ttsettingsbtnroleodeledit",
    description: "Destek talebine onay/ret veren rolü (kaldırır) günceller",
    cooldown: 5,
    async execute(interaction) {
        ...
        await Ticket_Schema.findOne({ guild_id: interaction.guild.id })
            .then(async schema => {
                ...
                const edited = deleteOrNodelete(schema.roleconfirm, roleId);
                schema.roleconfirm = edited;
                await schema.save();

                response
                    .setDescription(`**Destek talebine onay/ret veren rol** başarılı bir şekilde kaldırılarak güncellendi.\n\n${roleId}`)
                    .setColor("Green")
                interaction.update({ embeds: [response], components: [row] }).catch((error) => { error.message });
            })
            .catch((error) => {
