...
export const data = {
    name: "ttsettingsbtnroleoaddedit",
    description: "Destek talebine onay/ret veren rolü (ekle) günceller",
    cooldown: 5,
    async execute(interaction) {
        ...
        await Ticket_Schema.findOne({ guild_id: interaction.guild.id })
            .then(async schema => {
                ...
                const edited = addOrNoadd(schema.roleconfirm, roleId);
                schema.roleconfirm = edited;
                await schema.save();

                response
                    .setDescription(`**Destek talebine onay/ret veren rol** başarılı bir şekilde eklenerek güncellendi.\n\n${roleId}`)
                    .setColor("Green")
                interaction.update({ embeds: [response], components: [row] }).catch((error) => { error.message });
            })
            .catch(async (error) => {
                response
                    ...
                    .setColor("Green")
                interaction.update({ embeds: [response], components: [row] }).catch((error) => { error.message });
                await Ticket_Schema.updateOne({ guild_id: interaction.guild.id }, { $set: { roleconfirm: roleId } });
            });
