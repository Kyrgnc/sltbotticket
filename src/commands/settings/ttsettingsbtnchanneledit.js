...
export const data = {
    name: "ttsettingsbtnchanneledit",
    description: "Destek taleplerinin onaylanacağı kanalı günceller",
    cooldown: 5,
    async execute(interaction) {
        ...
        await Ticket_Schema.updateOne({ guild_id: interaction.guild.id }, { $set: { channelconfirm: channelId } }, { upsert: true })
            .then(() => {
                response
                    .setDescription(`**Destek talepleri onay kanalı** başarılı bir şekilde güncellendi.\n\n${channelId}`)
                    .setColor("Green")
                interaction.update({ embeds: [response], components: [row] }).catch((error) => { error.message });
            })
            .catch((error) => {
